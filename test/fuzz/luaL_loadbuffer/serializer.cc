...
public:
	...
	void step_out()
	{
		...
		if (block_stack_.top() == BlockType::Returnable) {
	...
	bool break_is_possible()
	{
		return !block_stack_.empty() &&
			block_stack_.top() == BlockType::Breakable;
	}
...
private:
	...
	/*
	 * Returnable block can be exited with return from
	 * breakable block within it, but breakable block cannot be
	 * exited with break from returnable block within it.
	 * Valid code:
...
PROTO_TOSTRING(WhileCycle, whilecycle)
{
	GetContext().step_in(Context::BlockType::Breakable);
...
PROTO_TOSTRING(Function, func)
{
	GetContext().step_in(Context::BlockType::Returnable);
